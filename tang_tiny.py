...
# —— 关键：把模型砍到极小 ——
BLOCK_SIZE, EMBED_DIM, N_HEADS, N_LAYERS = 64, 8, 2, 1   # 原来是 128 维 4 层，现在 8 维 1 层
STEPS, LR, BATCH = 2000, 3e-3, 64
...
m=M()
print(f"超小模型参数量：{sum(p.numel() for p in m.parameters()):,}（对比：原 tang 模型 87 万）\n")
opt=torch.optim.AdamW(m.parameters(),lr=LR)
for step in range(STEPS):
    xb,yb=batch(); _,loss=m(xb,yb); opt.zero_grad(); loss.backward(); opt.step()
    if step%400==0 or step==STEPS-1: print(f"step {step:4d} | loss: {loss.item():.4f}")
print("\n让这个超小模型作诗（看它吐的是不是乱码）：")
for i in range(5): print(f"第{i+1}首：{m.compose()}")
